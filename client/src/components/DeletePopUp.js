...
function DeletePopUp({ todoData, setTodoData, deletePopUp, setDeletePopUp }) {
  ...
  async function handleDeleteTask() {
    ...
    try {
      ...
      setDeletePopUp({ display: false, taskId: "", todoId: "", type: "" });
    } catch (error) {
        ...
        >
          <div className="relative w-full max-w-md h-full md:h-auto">
            <div className="relative  rounded-lg shadow bg-gray-700">
              <button
                type="button"
                onClick={() => setDeletePopUp({ display: false })}
                className="absolute top-3 right-2.5   rounded-lg text-sm p-1.5 ml-auto inline-flex items-center hover:bg-gray-800 hover:text-white"
